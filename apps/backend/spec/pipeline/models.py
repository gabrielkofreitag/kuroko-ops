...
def get_specs_dir(project_dir: Path) -> Path:
    ...
    IMPORTANT: Only .kuroko-ops/ is considered an "installed" kuroko-ops.
    The kuroko-ops/ folder (if it exists) is SOURCE CODE being developed,
    not an installation. This allows Auto Claude to be used to develop itself.

    This function also ensures .kuroko-ops is added to .gitignore on first use.

    Args:
        project_dir: The project root directory

    Returns:
        Path to the specs directory within .kuroko-ops/
    """
    # Initialize .kuroko-ops directory and ensure it's in .gitignore
    init_auto_claude_dir(project_dir)

    # Return the specs directory path
    return project_dir / ".kuroko-ops" / "specs"
